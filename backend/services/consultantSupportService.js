...
const createSupportResponse = async (req, res) => {
    try {
        const { response_id, request_id, emp_id, re_message, day } = req.body;

        if (!response_id || !request_id || !emp_id || !re_message) {
        ...
        try {
            ...
            res.status(201).json({ message: "Phản hồi đã được gửi và trạng thái được cập nhật thành công" });
        } catch (err) {
