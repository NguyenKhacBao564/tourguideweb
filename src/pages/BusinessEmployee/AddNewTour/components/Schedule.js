...
function Schedule(props) {
    ...
    const {schedule, onDeleteSchedule} = props;

    const { attributes, listeners, setNodeRef, transition, transform, isDragging } = useSortable({ id: schedule.id });
    ...
    return ( 
      ...
      style={style}
    >
      <GiHamburgerMenu size={30} className="schedule-icon" {...listeners}/>
      <Container className="schedule-content">
        ...
        <Button variant="light" className="bg-white">
          <AiFillEdit size={30} color="#F49B33" />
        </Button>
        <Button variant="danger" className="bg-white" onClick={() => onDeleteSchedule(schedule.id)}>
